...
function DropdownButtonHover(props) {
	const { items = [], active = 0, changeSelection, direction = "left-down" } = props;
	...
	return (
		...
		>
			<div className='mt-1 relative'>
				<button type='button' className={`relative w-full bg-white rounded-md shadow-dropdownRelative pl-3 pr-10 py-3 text-left cursor-pointer outline-none ${show ? " ring-1 ring-primary border-primary" : ""}`}>
							...
							{items.map((item, key) => {
								return (
									<li
										...
										role='option'
										className='cursor-pointer text-gray-900  select-none hover:bg-primary hover:text-white relative py-2 pl-3 pr-9 '
									>
...
export default DropdownButtonHover;
